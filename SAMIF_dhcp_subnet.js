...
for( var i in dhcp_subnet_Name)
{
	MasterPropertyMap[dhcp_subnet_Name[i]] = i;
	ReverseMasterPropertyMap[dhcp_subnet_Name[i]] = i;
	reverse_dhcp_subnet_Name[dhcp_subnet_Name[i]] = i;
}

function process_dhcp_subnet(samObject, modelInterface, className)
{
	
	logP3Msg("process_dhcp_subnet", "SAMIF"  , "entered into process_dhcp_subnet ");
    var element;
    var subelement;
    var siteName;
    ...
    logP1Msg("process_dhcp_subnet", "SAMIF",  "Valu of localDhcpName : " + localDhcpName );
    if (localDhcpName.indexOf("-") != -1){
    	logP1Msg("process_dhcp_subnet", "SAMIF",  "Entered in Telus Fix" );
    ...
    subelement.family = "5620_SAM_Subnet_Address";
    ...
    subelement.addProperty("samFamilyName", "5620_SAM_Subnet_Address");
    ...
    for( var i in dhcp_subnet_Name) 
    {
    	 if(i.toString() == "samSiteId" && samObject.nodeId != undefined){
			 ...
			 if (isDef(siteName)){
				 logP1Msg("process_dhcp_subnet", "SAMIF",  "Value of siteName : "+ siteName);
				 subelement.addProperty("samSiteName", siteName);
			 } 
		 }else if ( samObject[ dhcp_subnet_Name[i] ] != undefined ){
    		 subelement.addProperty( i.toString(), samObject[ dhcp_subnet_Name[i] ].toString() );
        }
    }
    
    if (isConfig("inv_uses_names"))
    {
    	subelement.label = siteName+"_DHCP4Server:"+samObject.localDhcpServerName+"_DHCPPool:"+samObject.addressPoolName+"_"+samObject.address;
    }
    else
    {
    	subelement.label = siteName+"_DHCP4Server:"+samObject.localDhcpServerName+"_DHCPPool:"+samObject.addressPoolName+"_"+samObject.address;
...
function jms_delete_dhcp_subnet(record)
{
    //var myId;
...
function jms_speed_update_dhcp_subnet(objectName, propColl)
{
    ...
    //logP4Msg("jms_speed_update_tdmequipment_DS1E1_PortSpecific", "SAMIF", "myId = "+myId);
	    ...
	    for( i in propColl)
	    {
	        ...
	        if (propName == "speed")
	        {
                ...
                logP4Msg("jms_speed_update_dhcp_subnet", "SAMIF", "samSpeed old value " + propOldValue);
                logP4Msg("jms_speed_update_dhcp_subnet", "SAMIF", "samSpeed new value " + propValue);
	        }
        }
}
