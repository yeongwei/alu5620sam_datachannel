...
for( var i in lte_S8Peer_Name)
{
	MasterPropertyMap[lte_S8Peer_Name[i]] = i;
	ReverseMasterPropertyMap[lte_S8Peer_Name[i]] = i;
	reverse_lte_S8Peer_Name[lte_S8Peer_Name[i]] = i;
}

function lte_S8Peer(samObject, modelInterface, className)
{
	logP4Msg("lte_S8Peer", "SAMIF", "entered lte_S8Peer" );
    ...
    subelement.family = "5620_SAM_S8Peer";
    ...
    subelement.addProperty("samFamilyName", "5620_SAM_S8Peer");

    for( var i in lte_S8Peer_Name) 
    {
    	if ( samObject[ lte_S8Peer_Name[i] ] != undefined )
        {
                subelement.addProperty( i.toString(), samObject[ lte_S8Peer_Name[i] ].toString() );
                logP4Msg("lte_S8Peer", "addProperty", "property: " + i.toString() +" value: "+ samObject[ lte_S8Peer_Name[i] ].toString() );
	    }
    }
...
function jms_delete_lte_S8Peer(record)
{
    turn_off_SE(record);
}
