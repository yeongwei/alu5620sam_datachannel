...
var reverse_equipment_mcmcard_Name = {},
equipment_mcmcard_Name =
{
...
for( var i in equipment_mcmcard_Name)
{
	MasterPropertyMap[equipment_mcmcard_Name[i]] = i;
	ReverseMasterPropertyMap[equipment_mcmcard_Name[i]] = i;
	reverse_equipment_mcmcard_Name[equipment_mcmcard_Name[i]] = i;
}

function equipment_mcmcard(samObject, modelInterface, className)
{
	
	logP3Msg("equipment_mcmcard", "SAMIF" , "entered equipment_mcmcard");
    var element;
    var subelement;
    ...
    subelement.family = "5620_SAM_MCMCard";
    ...
    subelement.addProperty("samFamilyName", "5620_SAM_MCMCard");

    for( var i in equipment_mcmcard_Name) 
    {
    	logP3Msg("PRASHANTMCMCARD", "PRPERTYNAME : " + i.toString() , "Value : "+samObject[ equipment_mcmcard_Name[i] ].toString());
    	if ( samObject[ equipment_mcmcard_Name[i] ] != undefined )
        {
                subelement.addProperty( i.toString(), samObject[ equipment_mcmcard_Name[i] ].toString() );
	    }
    }
...
function jms_delete_equipment_mcmcard(record)
{
    //var myId;
...
function jms_speed_update_equipment_mcmcard(objectName, propColl)
{
    ...
    logP4Msg("jms_speed_update_equipment_mcmcard", "SAMIF", "object: "+objectName);	
    //logP4Msg("jms_speed_update_tdmequipment_DS1E1_PortSpecific", "SAMIF", "myId = "+myId);
	    ...
	    for( i in propColl)
	    {
	        ...
	        if (propName == "speed")
	        {
                ...
                logP4Msg("jms_speed_update_equipment_mcmcard", "SAMIF", "samSpeed old value " + propOldValue);
                logP4Msg("jms_speed_update_equipment_mcmcard", "SAMIF", "samSpeed new value " + propValue);
	        }
        }
}
