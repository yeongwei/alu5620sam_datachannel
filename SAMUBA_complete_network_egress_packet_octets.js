...
//
//                p r o c e s s _ c o m p l e t e _ n e t w o r k _ e g r e s s _ p a c k e t _ o c t e t s _ l o g _ r e c o r d
//...............................................................................................
// function to loop over the name defined above and create a metric object for each entry
//
function process_complete_network_egress_packet_octets_log_record(record)
{
    var myId;
    var idmapname;
    var subelement;
	...
	logP4Msg("process_complete_network_egress_packet_octets_log_record", "record.queueId", record.queueId);
    var egressNetworkQueueId = record.queueId;
    if(isUndef(egressNetworkQueueId))
    {
	    logP4Msg("process_complete_network_egress_packet_octets_log_record", "SAMUBA_complete_network_egress_packet_octets", "Skipping null queueId");
	    return;
    }
    else
    {
	...
	logP4Msg("process_complete_network_egress_packet_octets_log_record", "monitoredObjectPointer",monObjPointer);
...
logP4Msg("process_complete_network_egress_packet_octets_log_record", "idmapname", idmapname);
	...
	{ 
		logP5Msg("process_complete_network_egress_packet_octets_log_record", "SAMUBA_complete_network_egress_packet_octets", "Skipping null subelement for --> "+ idmapname);
		return;
	}

	myId = subelement.id;  
    logP4Msg("process_complete_network_egress_packet_octets_log_record", "SAMUBA_complete_network_egress_packet_octets", "ENTERING for --> "+ record.monitoredObjectPointer + " with id == " + myId);
    ...
    for(var i = 0; i < nqueueCombinedNetworkEgressMetrics.length; i++)
	{
		...
		if (!(value == null || isNaN(value))) 
		{
			...
			CreateSimpleMetricObject(myId, record, "AP~Specific~Alcatel~5620 SAM~Bulk~equipment~NetworkQueues~Egress~", nqueueCombinedNetworkIngressMetrics[i]);
		}
	}
}
