...
var reverse_equipment_daughtercard_Name = {},
equipment_daughtercard_Name =
{
...
for( var i in equipment_daughtercard_Name)
{
	MasterPropertyMap[equipment_daughtercard_Name[i]] = i;
	ReverseMasterPropertyMap[equipment_daughtercard_Name[i]] = i;
	reverse_equipment_daughtercard_Name[equipment_daughtercard_Name[i]] = i;
}

function equipment_daughtercard(samObject, modelInterface, className)
{
	
	logP3Msg("equipment_daughtercard", "SAMIF" , "entered equipment_daughtercard");
    var element;
    var subelement;
    ...
    subelement.family = "5620_SAM_DaughterCard";
    ...
    subelement.addProperty("samFamilyName", "5620_SAM_DaughterCard");

    for( var i in equipment_daughtercard_Name) 
    {
    	logP3Msg("equipment_daughtercard", "addProperty", "property: " + i.toString() , " value: "+samObject[ equipment_daughtercard_Name[i] ].toString());
    	if ( samObject[ equipment_daughtercard_Name[i] ] != undefined )
        {
                subelement.addProperty( i.toString(), samObject[ equipment_daughtercard_Name[i] ].toString() );
	    }
    }
...
function jms_delete_equipment_daughtercard(record)
{
    //var myId;
...
function jms_speed_update_equipment_daughtercard(objectName, propColl)
{
    ...
    logP4Msg("jms_speed_update_equipment_daughtercard", "SAMIF", "object: "+objectName);	
    //logP4Msg("jms_speed_update_tdmequipment_DS1E1_PortSpecific", "SAMIF", "myId = "+myId);
	    ...
	    for( i in propColl)
	    {
	        ...
	        if (propName == "speed")
	        {
                ...
                logP4Msg("jms_speed_update_equipment_daughtercard", "SAMIF", "samSpeed old value " + propOldValue);
                logP4Msg("jms_speed_update_equipment_daughtercard", "SAMIF", "samSpeed new value " + propValue);
	        }
        }
}
